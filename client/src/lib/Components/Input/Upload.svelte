...
<div class="relative h-full w-full">
	...
	>
		...
		<div class="flex items-center justify-between relative h-full">
			<Text class="italic p-4 {!isValid ? 'text-altTextBrown' : 'text-placeholderGrayText'}"
				>{placeholder}</Text
			>
			<UploadIcon class="ml-2 w-8 h-8 {!isValid ? 'text-altTextBrown' : ' text-primaryYellow'}" />
		</div>
		...
		{#if hovering}
			...
			>
				<UploadIcon
					class="mb-2 text-placeholderGrayText w-full h-8 {!isValid && 'text-altTextBrown'}"
				/>
				<SmallText
					class=" text-placeholderGrayText text-center 
						{!isValid && 'text-altTextBrown'}"
				>
					Upload...
				</SmallText>
			</div>
	...
	{#if fileError}
		...
		>
			...
			<SmallText class="smallText text-red-500 text-center {!isValid && 'text-altTextBrown'}">
