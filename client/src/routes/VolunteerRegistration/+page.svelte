<script>
  ...
  import CalendarIcon from 'svelte-material-icons/CalendarMonthOutline.svelte';
  ...
  const validateForm = () => {
    formValid = Boolean(firstName) && Boolean(lastName) && Boolean(email) && phoneValid && Boolean(dob) && Boolean(password) && Boolean(reenterPassword) && termsAgreed && liabilityAgreed && passwordsMatch && validateEmail(email);
  };
